...
class SwiftUtilsV1:
    ...
    def get_app_performance(self, number_of_runs: int = 1):
        ...
        for i in range(number_of_runs + 1):
            ...
            if not result:
                print("Deploy error")
                error_str = f'ios-deploy -m --bundle_id {self.bundle_id} -b {self.base_dir}/launching.app -L -u'
                ...
                try:
                    ...
                    time.sleep(0.001)
                    r = requests.get(f"{self.server_url}getMeasurement", timeout=3600)
                    ...
                    if i != 0:
                        ...
                        np.array(ec_measurements).tofile(
                            f'./ec_measurements_partial.csv', sep=',')
                        np.array(runtime_measurements).tofile(
                            f'./time_measurements_partial.csv',
                            sep=',')
